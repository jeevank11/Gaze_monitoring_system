...
<style>
  ...
  /* Do NOT override the icon font on Material Symbol spans, otherwise the
     literal glyph name (e.g. "keyboard_double_arrow_left" for the sidebar
     collapse arrow) leaks through as plain text. */
  ...
  /* Icon-name-as-text fallback: some corporate networks strip Google Fonts
     requests, so Material Symbol spans render their literal glyph name
     ("keyboard_double_arrow_left", "arrow_downward", "content_copy", etc)
     as plain text. Collapse them to zero width so the raw text doesn't
     leak into the UI. Users lose the decorative icons but every button
     stays clickable and every layout stays intact. */
  .material-symbols-outlined,
  .material-symbols-rounded,
  .material-symbols-sharp,
  .material-icons,
